...
class placesController {  
    ...
    async apiMultiLocation(request) {
        ...
        try {
            let placeList = await this.facebookPlaces(accessToken , centerLat, centerLng, radius)            
            let picsList = await this.instagramMultiLocationPics(placeList)
            return defaultResponse(picsList)
        } catch (e) {
            return errorResponse(e.message)
        }
    }
        ...
        return axios.get(`https://www.instagram.com/explore/locations/${location.id}/`)
            .then(result => {      
                ...
                let picsArrayFiltered = picsArray.map(node => {
                    let cleanNode = _.pick(node,['id', 'date', 'thumbnail_src', 'caption', 'code', 'owner.id', 'comments.count', 'likes.count'])
                    cleanNode.location = {
    ...
    instagramMultiLocationPics(data) {
        ...
        return Promise.all(data.map(place => this.instagramSingleLocationPics(place, true)))
    ...
    instagramPicInfo(id) {
        return axios.get(`https://www.instagram.com/p/${id}/`)
            .then(result => {      
                let html = result.data
                let startMark = 'window._sharedData = '
                let endMark1 = 'show_app_install": true}'
    ...
    instagramTagInfo(tag) {
        return axios.get(`https://www.instagram.com/explore/tags/${tag}/`)
            .then(result => {      
                let html = result.data
                let startMark = 'window._sharedData = '
                let endMark1 = 'show_app_install": true}'
